...
local custom_server_config_factories = {
	---@param default_on_attach fun(client, bufnr)
	null_ls = function(default_on_attach)
	...
	---@param default_config table { on_attach, capabilities }
...
M.create = function()
	...
	for _, name in ipairs(default_server_names) do
		server_configs[name] = {
			get_server = function()
				return get_default_language_server(name)
			end,
			get_config = function()
	...
	for name, create in pairs(custom_server_config_factories) do
		if name == "typescript_tools" or name == "lua_ls" then
			server_configs[name] = create(default_config)
		end
		if name == "null_ls" then
			server_configs[name] = create(default_config.on_attach)
		end
	end
