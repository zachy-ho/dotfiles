-- Expand
map({ "i", "s" }, "<C-j>", "vsnip#expandable() ? '<Plug>(vsnip-expand)' : '<C-j>'", { noremap = false, expr = true })
...
map({ "i", "s" }, "<Tab>", "vsnip#jumpable(1) ? '<Plug>(vsnip-jump-next)' : '<Tab>'", { noremap = false, expr = true })
map(
	{ "i", "s" },
	"<S-Tab>",
